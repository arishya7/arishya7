## Arishya_hereee 😆
I'm an undergraduate student in Computer Science & Design at the Singapore University of Technology and Design (SUTD), passionate about building AI-powered, sustainability-driven, and human-centric systems.
I enjoy working at the intersection of software engineering, AI, and real-world impact — from climate-aware navigation and mental health AI research to full-stack dashboards and embedded systems.

## 🔭 I'm currently working on ...

### HealthTech
**CareTech – Wearable Knee Device for Elderly Rehabilitation**
Building an ML-based calibration pipeline to correct sensor drift in wearable movement-tracking devices, alongside user research across hospitals and eldercare centres to shape device design and clinical workflows.

### Social / Game Dev
**Somewhere – Social Discovery RPG**
A Club Penguin-style social app helping Singapore university students meet and connect, built in Godot with a Supabase backend.

## 👩‍💻 Technical Skills 
**Programming:**
Python, Java, JavaScript, SQL, HTML/CSS

**Frameworks & Platforms:**
FastAPI, Express, Node, React, Firebase, Android Studio, MySQL Workbench, Godot

**AI & Data:**
OpenAI & Gemini APIs, Spark, OpenCV, LLM evaluation & prompt engineering

**Cloud & DevOps:**
Docker, cloud deployment (Render / Fly.io), CI/CD fundamentals

**Embedded Systems:**
ESP32, Arduino IDE

**Design & Creative:**
Figma, Bubble.io, Adobe Creative Suite, Procreate, CapCut

## 📫 How to reach me:
  Email: arishya_jindal@mymail.sutd.edu.sg
  LinkedIn: [www.linkedin.com/in/arishya-j-365aa1293](https://www.linkedin.com/in/arishya-j-365aa1293)
